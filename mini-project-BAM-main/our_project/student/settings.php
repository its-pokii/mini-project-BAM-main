...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['save_profile'])) {
        ...
        if (!empty($_FILES['profile_photo']['name'])) {
            ...
            if (in_array($mime, $allowed)) {
                ...
                $filename = 'uploads/photos/' . $user_id . '_' . time() . '.' . $ext;
                if (move_uploaded_file($_FILES['profile_photo']['tmp_name'], $filename)) {
                    $photo_path = $filename;
                    $connector->prepare("UPDATE users SET profile_photo = ? WHERE id = ?")
                        ->execute([$photo_path, $user_id]);
                }
            } else {
        ...
        if ($check->fetch()) {
            $connector->prepare("
                ...
                WHERE user_id = ?
            ")->execute([$bio, $major, $current_year, $linkedin_url, $github_url, $other_url, $user_id]);
        } else {
            $connector->prepare("
                INSERT INTO student_profiles (user_id, bio, major, current_year, linkedin_url, github_url, other_url)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ")->execute([$user_id, $bio, $major, $current_year, $linkedin_url, $github_url, $other_url]);
        }
    ...
    if (isset($_POST['update_password'])) {
        ...
        if (!password_verify($current, $hash)) {
            ...
            $connector->prepare("UPDATE users SET password = ? WHERE id = ?")
    ...
    if (isset($_POST['save_prefs'])) {
        ...
        $connector->prepare("UPDATE users SET email_notif = ?, push_notif = ? WHERE id = ?")
            ->execute([$email_notif, $push_notif, $user_id]);
        $success = "prefs_saved";
    }

    // --- Delete Account ---
    if (isset($_POST['delete_account'])) {
        $connector->prepare("DELETE FROM student_profiles WHERE user_id = ?")->execute([$user_id]);
        $connector->prepare("DELETE FROM users WHERE id = ?")->execute([$user_id]);
        session_destroy();
